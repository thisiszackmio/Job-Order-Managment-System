    ...
    .then(() => {
      ...
      setPopupMessage(
        <div>
          ...
          <p>The new user was added to the database</p>
        </div>
      );
    })
    .catch((error) => {
      if (error.response && error.response.status === 400) {
        ...
        setPopupMessage(
          <div>
            <p className="popup-title">Form Validation</p>
            <p>{error.response.data.message}</p>
          </div>
        );
      } 
      else if (error.response.data.errors.image.includes('The image field must be a file of type: png.')) {
        setShowPopup(true);
        setPopupContent('error');
        setPopupMessage(
          <div>
            <p className="popup-title">Form Validation</p>
            <p>Image format must be png</p>
          </div>
        );
      } 
