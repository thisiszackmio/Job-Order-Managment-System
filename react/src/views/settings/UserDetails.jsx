...
export default function UserDetailsJLMS(){
        ...
        setPopupMessage(
          <div>
            ...
            <p className="popup-message">
              {responseErrors.firstname ||
              responseErrors.lastname ?
              "Empty fields on Name" : 
              responseErrors.position ? "Empty field on position" : null }
            </p>
          </div>
        );
        setShowPopup(true);
        ...
        if (error.response && error.response.status === 404) {
          ...
          setPopupMessage(
            <div>
              <p className="popup-title">Error</p>
              <p className="popup-message">
                {responseErrors.username ? "Empty on username" : 
                responseErrors.password ? "Please input Password" : null}
              </p>
            </div>
          );
        setShowPopup(true);
  ...
  const handleDeleteConfirmation = () => {
    ...
    setPopupMessage(
      <div>
        ...
        <p className="popup-message">Confirming will disable this account. Only the Site Admin can restore it. The account will be permanently deleted in 90 days.</p>
      </div>
    );
  }
  ...
  function GenerateId(id){
    ...
    .then(() => {
      ...
      setPopupMessage(
        <div>
          <p className="popup-title">Success</p>
          <p className="popup-message">Generate ID successfully</p>
        </div>
      );
    })
    .catch((error) => {
  ...
  const getRemainingDays = (expireDate) => {
    ...
    finalExpiry.setDate(finalExpiry.getDate() + 90);

    return Math.ceil((finalExpiry - new Date()) / (1000 * 60 * 60 * 24));
  };

  const remaining = getRemainingDays(userDet?.expire);

  // Restrictions Condition
  const ucode = currentUserCode;
  const codes = ucode.split(',').map(code => code.trim());
  const SuperAdmin = codes.includes("HACK");
  const ITAuthority = codes.includes("AUS");
            ...
            <div className="flex items-center mt-2">
              ...
              <div className="flex w-full">
                ...
                <>
                  ...
                  {!userDet?.userId && (userDet?.status === 1 || userDet?.status === 2) && (
                    <div className="w-1/4">
                      <div className="ml-2">
                        {/* Submit */}
                        <button 
                          onClick={() => GenerateId(userDet?.id)}
                ...
                {loading ? (
                  ...
                  enableDetail ? (
                    <div className="w-1/2">
                      <input
                        type="text"
                        className="focus:ring-0 w-full ppa-form-field h-[40px] border"
                        value={position}
                ...
                {loading ? (
                  ...
                  enableDetail ? (
                    <div className="w-1/2">
                      <select
                        type="text"
                        className="focus:ring-0 w-full ppa-form-field"
                        value={division}
                ...
                {loading ? (
                  ...
                  enableAccount ? (
                    <div className="w-1/2">
                      ...
                      <button
                        type="button"
                        className="absolute px-3 h-full"
            ...
            <>
              <form id="user_avatar" onSubmit={SubmitAvatar} method="POST" action="#" encType="multipart/form-data">
                <div className="items-center mt-2">
                  <div className="w-52 form-title-separate">
                    <label>
                      Upload Avatar:
                    </label> 
                  </div>
                  <div className="w-full">
                    <div className="mt-2 w-1/2 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6 py-1">
