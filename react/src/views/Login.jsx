...
export default function Login() {
  ...
  async function onUpdateLogin(ev) {
    if (ev) ev.preventDefault();

    setSubmitLoading(true);

    const UpdateCredentials = { username, currentPassword, newPassword, confirmPassword };

    try {
      await axiosClient.put("/updateuser", UpdateCredentials);

      setInputErrors('Password updated successfully');
      setChangePass(false);
      ...
      if (error.response?.status === 422) {
        if (message === 'Incorrect password') {
          ...
          message === 'The confirmPassword and newPassword must match.'
        ) {
      ...
      <div className="ppa-login-wrap">
        ...
        <div className="ppa-login-card">
          ...
          <div className="login-detail">
            ...
            {/* Error Code */}
            {inputErrors && ( <div className="login-error mb-2 mt-2"> {inputErrors} </div> )}
            {/* Login Details */}
            ...
            <>
              ...
              <button type="button" onClick={() => { onLogin();   ("exist"); }}  className={`px-4 py-2 w-full mt-6 text-sm ${ submitLoading ? 'btn-process' : 'btn-primary'}`} disabled={submitLoading}>
                Proceed
              </button>
              ...
              <form onSubmit={onUpdateLogin} className="mt-2 formlg-change" action="#" method="POST">
                ...
                <div className="relative">
                  <input
                    ...
                    className="block focus:ring-0 ppa-form-login"
                ...
                <div className="relative mt-2">
                  <input
                    ...
                    className="block w-full focus:ring-0 ppa-form-login"
                ...
                {/* Button */}
                <div className="mt-5">
                  <button type="submit" onClick={() => setChangeMethod("login")} className={`px-6 py-2 text-sm w-full ${ submitLoading ? 'btn-process' : 'btn-primary'}`} disabled={submitLoading}>
                    {submitLoading ? (
          ...
          <p className="system-version py-5">JOMS version 2.1</p>
        </div>
      </div>
    </div>
  </div>
